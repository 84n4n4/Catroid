...
import static org.catrobat.catroid.ui.recyclerview.backpack.BackpackActivity.FRAGMENT_SOUNDS;
...
public class SoundFragment extends RecyclerViewFragment<SoundInfo> {
	...
	protected void packItems(List<SoundInfo> selectedItems) {
		...
		try {
			for (SoundInfo item : selectedItems) {
				...
				SoundInfo itemForBackpack = new SoundInfo(item.getTitle(), fileName);
				itemForBackpack.isBackpackSoundInfo = true;
				BackPackListManager.getInstance().getBackPackedSounds().add(itemForBackpack);
			}
	...
	protected void switchToBackpack() {
		...
		intent.putExtra(FRAGMENT_POSITION, FRAGMENT_SOUNDS);
	...
	public Set<String> getScope() {
		...
		for (SoundInfo item : adapter.items) {
	...
	protected void copyItems(List<SoundInfo> selectedItems) {
		...
		for (SoundInfo item : selectedItems) {
			try {
				...
				adapter.items.add(new SoundInfo(name, fileName));
			} catch (IOException e) {
				Log.e(TAG, Log.getStackTraceString(e));
			}
		}
