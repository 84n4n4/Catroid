...
public class NfcTagFragment extends RecyclerViewFragment<NfcTagData> {
	...
	protected Set<String> getScope() {
		...
		for (NfcTagData item : adapter.items) {
	...
	protected void deleteItems(List<NfcTagData> selectedItems) {
		adapter.items.removeAll(selectedItems);
	}

	@Override
	protected void showRenameDialog(List<NfcTagData> selectedItems) {
		String name = selectedItems.get(0).getNfcTagName();
		RenameItemDialog dialog = new RenameItemDialog(R.string.rename_nfctag_dialog, R.string.new_nfc_tag, name,
				this);
