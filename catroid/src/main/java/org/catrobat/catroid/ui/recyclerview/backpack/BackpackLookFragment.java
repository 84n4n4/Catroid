...
public class BackpackLookFragment extends BackpackRecyclerViewFragment<LookData> {
	...
	protected void unpackItems(List<LookData> selectedItems) {
		...
		try {
			for (LookData item : selectedItems) {
				String name = uniqueNameProvider.getUniqueName(item.getLookName(), getDstScope());
				String fileName = StorageHandler.copyFile(item.getAbsolutePath(), getUnpackDstPath()).getName();
				getDstItemList().add(new LookData(name, fileName));
			}
	...
	@Override
	protected List<LookData> getDstItemList() {
		return ProjectManager.getInstance().getCurrentSprite().getLookDataList();
	}

	@Override
	protected Set<String> getDstScope() {
		Set<String> scope = new HashSet<>();
		for (LookData item : getDstItemList()) {
