...
import static org.catrobat.catroid.ui.recyclerview.backpack.BackpackActivity.FRAGMENT_LOOKS;
import static org.catrobat.catroid.ui.recyclerview.backpack.BackpackActivity.FRAGMENT_POSITION;
import static org.catrobat.catroid.utils.Utils.buildPath;

public class LookFragment extends RecyclerViewFragment<LookData> {
	...
	private static final String BACKPACK_DIRECTORY = buildPath(Constants.DEFAULT_ROOT,
			Constants.BACKPACK_DIRECTORY,
			Constants.BACKPACK_IMAGE_DIRECTORY);

	@Override
	protected void initializeAdapter() {
		List<LookData> items = ProjectManager.getInstance().getCurrentSprite().getLookDataList();
	...
	protected void packItems(List<LookData> selectedItems) {
		...
		try {
			for (LookData item : selectedItems) {
				String fileName = StorageHandler.copyFile(item.getAbsolutePath(), BACKPACK_DIRECTORY).getName();
				LookData itemForBackpack = new LookData(item.getLookName(), fileName);
				itemForBackpack.isBackpackLookData = true;
				BackPackListManager.getInstance().getBackPackedLooks().add(itemForBackpack);
			}
	...
	protected void switchToBackpack() {
		...
		intent.putExtra(FRAGMENT_POSITION, FRAGMENT_LOOKS);
	...
	protected Set<String> getScope() {
		...
		for (LookData item : adapter.items) {
	...
	protected void copyItems(List<LookData> selectedItems) {
		...
		for (LookData item : selectedItems) {
			try {
				String name = uniqueNameProvider.getUniqueName(item.getLookName(), getScope());
				String fileName = StorageHandler.copyFile(item.getAbsolutePath()).getName();
				adapter.items.add(new LookData(name, fileName));
			} catch (IOException e) {
				Log.e(TAG, Log.getStackTraceString(e));
			}
		}
	...
	protected void showRenameDialog(List<LookData> selectedItems) {
		String name = selectedItems.get(0).getLookName();
		RenameItemDialog dialog = new RenameItemDialog(
				R.string.rename_look_dialog,
				R.string.new_look_name, name,
				this);
	...
	public void renameItem(String name) {
		...
		if (!item.getLookName().equals(name)) {
			item.setLookName(name);
		}
		finishActionMode();
	}
